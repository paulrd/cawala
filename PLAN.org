...
* Decisions (resolved 2026-08-21)
  ...
  8. Node splitting: automatic split algorithm deferred to post-v1; moved
     ...
     with fresh addresses discovered out of band - no redirects). Splitting is
     a manual administrative operation - a node admin
  ...
  10. Signed topology/registry distribution: **rejected**. The live network
     topology is the source of truth: each node's authoritative parent/child
     links (healed by `Rebase`/`RebasePull` and ordered by the subtree routing
     epoch) already define routing and the address<->identity binding, so there
     is no distributed snapshot to sign, cache, refresh, revoke, or reconcile,
     and no equivocation surface to model. Distributing signed topology would add
     complicated caching dynamics for a prevention benefit the threat model does
     not require. Consequence: fabricated non-neighbor hops and address/key
     substitution remain **detectable** (audit, netting, carried-prefix Phase 2,
     `EdgeClose`) but not **prevented** - consistent with the accepted risks
     ("nodes are intentionally not zero-trust"). This also removes the general
