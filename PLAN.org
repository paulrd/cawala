...
* Milestones (risk-sequenced)
  ...
  2. *M1 - Topology & addressing* (DONE 2026-08-21): octal address scheme,
   ...
   5. *M4 - Control & web client* (PHASE 1 DONE 2026-09-10; USER-LIVE
      ...
      - Deferred: cross-subtree/multi-hop value movement (a browser order is
        same-leaf `Direct` only; paying a user under another leaf, cascade
        settlement, and tree-routed indirect multi-node control are all
        unimplemented); browser issue/burn (value creation/destruction stays an
        explicit operator-only act - `ledger fund` - and is deliberately not
        reachable from a browser order); and remote admin from the browser
        (pending-join listing + approve/reject would need a
        `CONTROL_FORMAT_VERSION` bump and a node operator-key custody
        decision). `MoveChild` remains limited to re-slotting under the same
        node; no control replay nonce (authority is re-evaluated live per
        request). The location suggestion service is out of scope for now: a
        separate, independently hostable, non-authoritative companion (lat/lon
        or map click -> suggested octal address); the leaf still validates and
