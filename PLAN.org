...
* Milestones (risk-sequenced)
  ...
  2. *M1 - Topology & addressing* (DONE 2026-08-21): octal address scheme,
   ...
   5. *M4 - Control & web client* (PHASE 1 DONE 2026-09-10; USER-LIVE
      WIRING DONE 2026-09-13; in progress):
      ...
      - Deferred: tree-routed indirect multi-node control; network issue/burn;
        remote admin from the browser (pending-join listing + approve/reject
        control requests would need a `CONTROL_FORMAT_VERSION` bump and a node
        operator-key custody decision); account/activity query over control;
        browser serving `cawala/msg/0` as a joined node. `MoveChild` remains
        limited to re-slotting under the same node; no control replay nonce
        (authority is re-evaluated live per request). The location suggestion
        service is out of scope for now: a separate, independently hostable,
        non-authoritative companion (lat/lon or map click -> suggested octal
        address); the leaf still validates and issues the final address.
