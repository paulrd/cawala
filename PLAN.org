...
* Milestones (risk-sequenced)
  ...
  2. *M1 - Topology & addressing* (DONE 2026-08-21): octal address scheme,
   ...
   5. *M4 - Control & web client* (PHASE 1 DONE 2026-09-10; in progress):
      ...
      location SQLite service, join flow (parent approves -> leaf issues
      address).
      ...
      - PWA (foundation): Svelte 5 shell, dark design-token system, 14 shared
        components, hash router, rune stores, and a single `lib/api.js`
        adapter (mock mode) insulating the UI from the wasm/control surface;
        route pages present, live wiring pending.
      - Deferred: tree-routed indirect multi-node control; network issue/burn;
        wasm control exports + PWA live-data wiring; the location SQLite
        service (awaiting the operator-provided database/schema);
        `MoveChild` limited to re-slotting under the same node; no control
        replay nonce (authority is re-evaluated live per request).
