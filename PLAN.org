...
* Milestones (risk-sequenced)
  ...
  2. *M1 - Topology & addressing* (DONE 2026-08-21): octal address scheme,
   ...
   5. *M4 - Control & web client* (PHASE 1 DONE 2026-09-10; in progress):
      ...
      - Onboarding invite (`crates/control::invite`): a portable
        `cawala://join?parent=<EndpointId>&op=<operator-pubkey>[&slot&exp&label]`
        URI carrying the parent's EndpointId and a pinned operator key. The
        node CLI generates it (`control invite`) and consumes it
        (`control join --invite`); pinning the parent operator key closes the
        phase-1 trust-on-first-use gap on `JoinApproved`.
      - PWA (foundation): Svelte 5 shell, dark design-token system, 14 shared
        components, hash router, rune stores, a single `lib/api.js` adapter
        (mock mode) insulating the UI from the wasm/control surface, and an
        invite-entry join flow; route pages present, live wiring pending.
      - Deferred: tree-routed indirect multi-node control; network issue/burn;
        wasm control exports + PWA live-data wiring; `MoveChild` limited to
        re-slotting under the same node; no control replay nonce (authority is
        re-evaluated live per request). The location suggestion service is out
        of scope for now: a separate, independently hostable, non-authoritative
        companion (lat/lon or map click -> suggested octal address); the leaf
        still validates and issues the final address.
