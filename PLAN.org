...
* Milestones (risk-sequenced)
  ...
  2. *M1 - Topology & addressing* (DONE 2026-08-21): octal address scheme,
   ...
   5. *M4 - Control & web client* (PHASE 1 DONE 2026-09-10; USER-LIVE
      ...
      - Still owed: result-path signature verification (settlement results
        remain advisory - a rogue parent/LCA can misreport but cannot move
        funds); route-wide carried-prefix verification beyond direct neighbors;
        commitment-chain assembly + `net`/`verify_cascade` operator harness;
        no global supply anchor by design; manual prefund liquidity (an
        underfunded LCA parent yields a `Partial`: payer debited, payee not
        credited); control-format v2 is a hard fleet break.
