...
* Milestones (risk-sequenced)
  ...
  2. *M1 - Topology & addressing* (DONE 2026-08-21): octal address scheme,
   ...
   5. *M4 - Control & web client* (PHASE 1 DONE 2026-09-10; USER-LIVE
      ...
      - Cross-subtree hardening still owed (P5): parent<->child ledger-key
        distribution + full carried-prefix signature verification (settlement
        results are advisory - a rogue LCA/parent can misreport but cannot move
        funds); commitment-chain assembly + `net`/`verify_cascade` operator
        tooling; the `MirrorMismatch` direction split; no global supply anchor
        by design; manual prefund liquidity (an underfunded LCA parent yields a
        `Partial`: payer debited, payee not credited).
