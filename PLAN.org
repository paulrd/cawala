...
* Milestones (risk-sequenced)
  ...
  6. *M5 - Hardening*: exit rights, foster-parent recovery, compromise-path
     ...
     - Companion gap (DONE): `JoinRejected` now requires the invite-pinned
       ...
       wasm `LocalState1::on_join_rejected`), mirroring `JoinApproved`; a spoofed
       self-signed rejection can no longer cancel an in-flight join (the outbound
       join is left intact on mismatch). Deferred: a `rejection.nonce` staleness
       check (edge case when the parent has no pending row, so its nonce is 0).
